...
public class ULLMap<K, V>  implements Map61B<K, V> {
    ...
    public V remove(K key) {
        ...
        // Remove pointers to the Entry with (key). Java will handle garbage collection.
        size--;
        if (list.key.equals(key)) {
            list = list.next;
        } else {
            Entry currEntry = list;
            while (!currEntry.next.key.equals(key)) { currEntry = currEntry.next; }
    ...
    public V remove(K key, V value) {
        V valFound = get(key);
        if (valFound == null) return null;
        if (valFound.equals(value)) return remove(key);
        else return null;
    }
